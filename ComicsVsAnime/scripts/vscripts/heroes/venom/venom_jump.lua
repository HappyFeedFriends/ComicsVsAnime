if venom_jump == nil then venom_jump = class({}) end

LinkLuaModifier( "modifier_venom_jump_leap", "heroes/venom/venom_jump.lua", LUA_MODIFIER_MOTION_BOTH )
--------------------------------------------------------------------------------
function venom_jump:OnAbilityPhaseStart()
    if IsServer() then
        self:GetCaster():StartGesture(ACT_DOTA_CAST_ABILITY_2_ES_ROLL_START)
    end
    return true
end
function venom_jump:OnSpellStart()
    if IsServer() then
        ...
        self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_venom_jump_leap", kv )
...
if modifier_venom_jump_leap == nil then 
	modifier_venom_jump_leap = class({}) 
end
...
function modifier_venom_jump_leap:IsStunDebuff()
    return true
end

--------------------------------------------------------------------------------

function modifier_venom_jump_leap:IsHidden()
    return true
end

--------------------------------------------------------------------------------

function modifier_venom_jump_leap:IsPurgable()
    return false
end

--------------------------------------------------------------------------------

function modifier_venom_jump_leap:RemoveOnDeath()
    return false
end

--------------------------------------------------------------------------------

function modifier_venom_jump_leap:OnCreated( kv )
    if IsServer() then
...
function modifier_venom_jump_leap:OnDestroy()
    if IsServer() then
	local caster = self:GetCaster()
	local radius = self:GetAbility():GetSpecialValueFor("radius")
	local target = self:GetParent()
	local damage = self:GetAbility():GetSpecialValueFor("damage")
	local talent = caster:FindAbilityByName("special_bonus_venom_1")
	...
	local move_slow = self:GetAbility():GetSpecialValueFor("move_slow")
		...
		local localUnits = FindUnitsInRadius(caster:GetTeamNumber(),
			...
			if talent and talent:GetLevel() > 0 then
						local radius = 50
						local particle = ParticleManager:CreateParticle( "particles/arena/econ/units/heroes/hero_stargazer/gamma_ray_sunray_ground_immortal1.vpcf", PATTACH_ABSORIGIN_FOLLOW, target );
				...
				local value_d = talent:GetSpecialValueFor("value")
				unit:AddNewModifier( caster, self:GetAbility(), "modifier_stunned", {duration = value_d}) 
...
function modifier_venom_jump_leap:DeclareFunctions()
    local funcs =
    {
...
function modifier_venom_jump_leap:CheckState()
    local state =
    {
...
function modifier_venom_jump_leap:UpdateHorizontalMotion( me, dt )
    if IsServer() then
...
function modifier_venom_jump_leap:UpdateVerticalMotion( me, dt )
    if IsServer() then
...
function modifier_venom_jump_leap:OnHorizontalMotionInterrupted()
    if IsServer() then
...
function modifier_venom_jump_leap:OnVerticalMotionInterrupted()
    if IsServer() then
		self:Destroy()
...
function modifier_venom_jump_leap:GetOverrideAnimation( params )
    return ACT_DOTA_CAST_ABILITY_2
end

function venom_jump:GetAbilityTextureName() return self.BaseClass.GetAbilityTextureName(self)  end 
