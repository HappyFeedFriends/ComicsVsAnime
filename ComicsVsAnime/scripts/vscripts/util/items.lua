function ComicsVsAnime:ThinkGoldDrop()
...
function ComicsVsAnime:SpawnGold()
...
function ComicsVsAnime:SpawnGoldEntity( spawnPoint )
...
function ComicsVsAnime:KillLoot( item, drop )
...
function ComicsVsAnime:SpecialItemAdd( event )
	...
	if GetTeamHeroKills( leader ) > 5 and GetTeamHeroKills( leader ) <= 20 then
	...
	elseif GetTeamHeroKills( leader ) > 20 and GetTeamHeroKills( leader ) <= 40 then
		if ownerTeam == leader and ( self.leadingTeamScore - self.runnerupTeamScore > 3 ) then
			spawnedItem = t2
		elseif ownerTeam == lastPlace then
			spawnedItem = t4
...
function ComicsVsAnime:ThinkSpecialItemDrop()
...
function ComicsVsAnime:PlanNextSpawn()
...
function ComicsVsAnime:WarnItem()
...
function ComicsVsAnime:SpawnItem()
	-- notify everyone
...
function ComicsVsAnime:ForceSpawnItem()
	self:WarnItem()
	self:SpawnItem()
end

function ComicsVsAnime:KnockBackFromTreasure( center, radius, knockback_duration, knockback_distance, knockback_height )
...
function ComicsVsAnime:TreasureDrop( treasureCourier )
...
function ComicsVsAnime:ForceSpawnGold()
	self:SpawnGold()
end
